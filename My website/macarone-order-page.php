...
<body>
	...
	<br>
	 ...
	 <!--ADDS ALL THE INFORMATION ABOUT Macarone -->
	 <!--.....................................................-->
	<!-- . This form tag sends data to macarone-order-page when "buy" button is pressed-->
	<!-- . "action" is where form data will be sent when "buy" button is pressed, so it will be sent in macarone-order-page.php -->
	<!-- . "method" is how data will be sent when "buy" button is pressed, so it will be sent without other people seeing it in macarone-order-page.php -->
	<form action="macarone-order-page.php" method="post" > 

		<div class ="macarone-info-container"> <!-- Collects all the elements that involves with macarone information, such as images, text and button -->
			<img src="FOODS/macarone.jpg" width="512" height="751" id="macarone" /> <!--Adds image of macarone by width of 512 and height of 751 -->
			<h1 class="macaroneText"> Macarone <br> </h1> <!-- Adds heading "Macarone" that adds new line (br) -->
			<h2> 10.00GPB <br> </h2> <!--Adds price of macarone that adds new line (br) -->
			<p> Enjoy the nice and cheesy taste of macarone </p> <!-- Adds little information about macarone -->
			...
			<div class="num-container"> <!--Collects elements that involves with quantity box (how many macarone they want to order) -->
				<input type="number" name="amount" min="0" max="5" step="1" value="0" id="input-num"> <!-- Adds input box which starts with 0 "Value" and max of macarone "max = 5" -->
			</div>
		</div>
	</form>
	...
	<div class="CookingInstructionContainer"> 	<!-- Collects elements that goes into cooking instruction section such as heading and lists -->
		<h1 class= "cookingInstruction" width="1920">Cooking Instruction </h1> 	<!-- Adds heading "Cooking Instruction" -->
		  <li class="CI1">Line a baking sheet with parchment paper</li> <!-- Adds lists of cooking instruction -->
		  <br></br> <!--Adds new line -->
		  <li class="CI2">Preheat the oven to 300°F (150°C). Bake the macarons for 15-18 <br> minutes, or until they are set but still slightly soft in the <br> center.</li> <!-- Adds lists of cooking instruction with new line (br) -->
		</div>
	  </div>
	  ...
	  <div class="IngredientsContainer"> <!-- Collects elements that goes into ingredients such as heading -->
		...
		<div class="lists"> <!-- Because i cannot style all the class in CSS, i have added a div class "lists" -->
		  <li>1 3/4 cups powdered sugar</li> <!-- Adds lists of ingredients -->
		  <li>1 cup almond flour</li> <!-- Adds lists of ingredients -->
		  <li>3 large egg whites, room temperature</li> <!-- Adds lists of ingredients -->
		  <li>1/4 cup granulated sugar</li> <!-- Adds lists of ingredients -->
		</div>
	  </div>
	...
	<!--................................................................................................................-->
	 ...
	 <style>
		...
		/* Styles div class "macarone-info-container" image */
		.macarone-info-container img{
			...
			left:58px; /*Position  left of the macarone image  "58" (PIXELS) */
			top:289px; /*Position  top of the macarone image  "289" (PIXELS) */
		}


		/* Styles div class "macarone-info-container" heading 1 */
		.macarone-info-container h1{
			position: absolute; /* Position: "Absolute" allows me to take advantage on the position of the elements  */
		...
		/* Styles div class "macarone-info-container" heading 2 */
		.macarone-info-container h2{
			position: absolute; /* Position: "Absolute" allows me to take advantage on the position of the elements  */
		...
		/* Styles div class "macarone-info-container" paragraph */
		.macarone-info-container p{
			position: absolute; /* Position: "Absolute" allows me to take advantage on the position of the elements  */
		...
		/* Styles div class "lists" which includes list of ingredients for macarone */
		.lists{
			position:absolute; /* Position: "Absolute" allows me to take advantage on the position of the elements  */
...
if (isset($_POST['buy'])) //it will check if the button "buy" is pressed
{
     ...
     $var2 = 10; //Var 2 collects the price of macarone which is £10
	 $var4 = null; //For now var 4 is null meaning there is nothing in the variable
	 $price = $var2*$var1; //price times the cost by the amount of macarone they buy
	 $var0 = null; //For now var 0 is null meaning there is nothing in the variable
	 $getstock =  "SELECT stock FROM items WHERE itemname = 'macarone'"; //getstock  is a variable that requests information from the database table "items" on the quantity of macarone that is currently in stock.
    ...
    $fin = mysqli_query($connect,$getstock) or die(mysqli_error()); //chooses the stock column from the items table where the itemname is "macarone" and is stored in the variable $getstock.
	...
	if (mysqli_num_rows($fin) > 0) { //It checks if there is any rows in the items table,which is used by function "mysqli_num_rows"
		while($row = mysqli_fetch_assoc($fin)) { //If there is rows it uses while loop and uses utilises the mysqli_fetch_assoc function to get each row one at a time.
			...
			$macarone = true; //macarone variable is set to true when the value is set
		}
	}
 
	if ($var1 <=$var0){ //Uses if statement to see if amount of macarone (var 1) is less than equal to stock (var0)
		$var4 = $var0 - $var1; //If it is then it uses var 4 to take away the stock in the database to amount 
		$sql = "UPDATE items 
		SET stock = '$var4'
		WHERE itemname = 'macarone'
		"; //It updates new stock amount after it takes away from the stock and set it to var 4 which taked away the amount by stock, it selects the row "macarone" which is added in database
	...
	}else{ //Uses else to say what the website will say when the macarone is out of stock.
		echo"
		<script>
